var application = application || (function ($) {
     ...
     *
     * - full namespace application.modules.ui.modal
     * - or module.ui.modal
     * - or short ui.modal
     *
     * Usage:
     *
     * application.module('ui.modal', function(module, require, $) {...});
     *
    ...
    var module = function (id, moduleFunction) {
        ...
        instance.id = 'application.modules.' + _cutModulePrefix(id);
     ...
     * or classes.
     *
     * For the module application.modules.ui.modal you can search:
     *
     * require('ui.modal');
     * require('modules.ui.modal');
     * require('application.modules.ui.modal');
     *
     * @param {type} moduleId
    ...
    var resolveNameSpace = function (typePath, init) {
        try {
            //cut application.modules prefix if present
    ...
    /**
     * Cuts the prefix application.modules or modules. from the given value.
     * @param {type} value
     * @returns {unresolved}
     */
    var _cutModulePrefix = function (value) {
        return _cutPrefix(_cutPrefix(value, 'application.'), 'modules.');
    };

    /**
     * Cuts a prefix from a string, this is already available in application.util but
    ...
    $(function() {
        ...
        event.trigger('application:ready');
        $(document).trigger('application:ready', [false, application]);
    });

    var initModule = function (module) {
        var log = require('log');
        event.trigger('application:beforeInitModule', module);
        if (module.init) {
            try {
        ...
        event.trigger('application:afterInitModule', module);
    ...
    event.on('application:modules:client:pjax:success', function (evt) {
        ...
        event.trigger('application:ready');
        $(document).trigger('application:ready', [true, application]);
    }).on('application:modules:client:pjax:beforeSend', function (evt) {
        unloaded = [];
        $.each(moduleArr, function (i, module) {
            if (module.unload) {
                module.unload();
